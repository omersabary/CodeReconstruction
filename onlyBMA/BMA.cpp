...
char BMAMajority(vector<string> &cluster, vector<int> &pointers){
    ...
    while(it!=cluster.end()){
        ...
        if(DNAtoInt(seq[pointers[current_seq]]) <= -1 || DNAtoInt(seq[pointers[current_seq]])-'0' >=4){
            cerr << "darlin " << " " << pointers[current_seq] << " " << seq[pointers[current_seq]] << endl;
            //throw MyException("INVALID DNA Char ");
            //cout << "ERROERERE  " << endl;
        }
        else{
            majority[DNAtoInt(seq[pointers[current_seq]])]++;
        }
        current_seq++;
        it++;
    }
...
int main()
{
    ...
    for(d_prob = 0.01 ; d_prob<=0.11; d_prob+=0.01){
        clus_size = 10;
        num_of_test=10000;
        cout << "Cluster Size: " << clus_size << endl;
        cout << "Prob: " << d_prob << endl;
        cout << "Actual Prob: " << (1-pow(1-d_prob,1)) << endl;
        ...
        while(nt<num_of_test){
            ...
            exit(0);*/
            createNoisyCluster(cluster, original, clus_size, d_prob);
           ...
           /* string karin = LOOKAhead_Majority(cluster, DES_LEN, 2);
            ...
            lookback = nlookback;*/
            int origSize=DES_LEN;
            string bma =BMA(cluster, DES_LEN, origSize);
            //cout << original << endl;
